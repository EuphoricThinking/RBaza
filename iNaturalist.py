...
def arguments():
    parser = argparse.ArgumentParser(description="The script for calculating Shannon-Wiener index, Simpson index, species "
                                                 "biodiversity, the number of observations", usage="Requires th csv file "+
                                                 "created by iNaturalist with default taxon specification and customized"+
                                                 "other labels: id, user_login, description in Basic and place_guess in Geo")
    ...
    for species in range(len(unrep_taxon)):                                     #Sums up to the number of the unique names of the organisms
        for row in orig_lines:                                                  #Checks every observation in the orginal file containing possible duplicates
            if row[5] in track:                                                 #If the species has not been counted
                ...
                sum_simpson = sum_simpson+(num_tax_one*(num_tax_one-1))         #Nominator of the Simpson factor
